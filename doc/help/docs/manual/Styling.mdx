...
The view you see when you first open the app is intentionally very simple, but hidden away are many more features.  To access these, click on the last, Settings, button on the top bar: <img
src={normalizeImagePath("/images/SettingsButton.png")}
alt="Settings Button"
style={{ width: 20, display: "inline" }}
/>.  This will reveal a panel with a set of four tabs: Factors, Links, Network and Analysis, with the Factors tab open.
...
<img
src={normalizeImagePath("/images/FactorsPanel.png")}
alt="Factors Panel"
style={{ width: 250}}
/>

There are 15 sample styles for how factors can look.  If you select a factor from the network and then click on one of the 15 styles, the factor will change to resemble the style.  As a short cut, if you click on the 'Select all factors' button at the bottom, and then click on a style, all the factors will change to the chosen style.

Double clicking on any of the 15 styles opens a dialog box to change the style:

<img
src={normalizeImagePath("/images/FactorsStyle.png")}
alt="Factors Style"
style={{ width: 600}}
/>

There are options to change the colour of the background (the 'fill'), the border and the font, to change the shape, for example to a rectangle or a circle, to change the border from solid to dashed or dotted or none, and to change the font size of the label.  Clicking on the padlock symbol will [lock](/manual/GettingStarted#notes) all the factors with this style; clicking on it again will unlock them all. The Size slider adjusts the relative size of the factors. Moving the dot on the slider to the far left sets the factors to their normal size, and moving it to the right makes the factors bigger.

Clicking on one of the coloured circles ('colour wells') brings up a dialog to choose a colour.

<img
src={normalizeImagePath("/images/ColourWheel.png")}
alt="Colour wheel"
style={{ width: 200}}
/>
...
<img
src={normalizeImagePath("/images/FactorsRightClick.png")}
alt="Factor Right Click menu"
style={{ width: 250}}
/>
...
<img
src={normalizeImagePath("/images/LinksPanel.png")}
alt="Links Panel"
style={{ width: 250}}
/>
...
<img
src={normalizeImagePath("/images/NetworkPanel.png")}
alt="Network Panel"
style={{ width: 250}}
/>
...
* **Show history** When ON, a panel displaying every change to a factor or link (adding a factor or link, editing it or deleting it) is shown. It is possible to 'rollback' the map to the state it was in before a change.  This can be quicker than repeatedly undoing changes with the Undo button.  To rollback, click on the <img src={normalizeImagePath('/images/bootstrap-reboot.svg')} alt="settings"
style={{display: "inline" }}/> button.  You will be asked to confirm the action.  The states of the map for the previous 10 changes are kept. Clicking on the clipboard icon <img
src={normalizeImagePath("/images/history-clipboard-icon.png")}
alt="Clipboard button"
style={{ width: 20, display: "inline" }}
/> copies the log to the clipboard, from where it can be pasted into a spreadsheet or word processor app.
...
<img
src={normalizeImagePath("/images/FactorReactions.png")}
alt="Reactions"
style={{ width: 250}}
/>
...
The Analysis tab allows you to view portions of the map and to cluster factors to help with the analysis of the network (see the [Analysis](/manual/Analysing#analysing-the-map) section for help on how this can be useful).
...
Clustering is done according to the values of  the clustering attribute - all factors with the same value for this attribute are joined into the same cluster.  The **Cluster** pull-down menu offers as standard, **Style** (i.e. the factors' style as set in the [Factors tab](#factors-tab)) and **Colour** (i.e. the colour of the factors' backgrounds) as possible attributes with which to cluster.  There is also clustering using the [**Louvain**](https://en.wikipedia.org/wiki/Louvain_method) algorithm which forms non-overlapping clusters of 'communities'. In addition, bespoke attributes can be used by creating a new column in the Data View and giving each factor a value there (see the section on the [Data View](/manual/DataView#data-view) for how to create attributes).  These additional attributes are automatically added to the Cluster pull-down menu when they are created.
