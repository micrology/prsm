...
* **New link** (or edge). To draw a link between two factors, click on the arrow button and then drag from one factor to another to link them. If you wish, you can have two links between a pair of factors: one for each direction.
...
* **Save**  Save the network in a file on your computer. The file is saved in the app's own format, or if you hover over the small triangle next to the button, you can choose to save an image of the map as a high resolution image (a .PNG file), as a Microsoft Excel file, or the map data in GML (Graph Markup Language) or several other formats. See [Reading and Saving](/manual/Files#saving-and-exporting-maps) for further details.
...
If you are not sure how to do something with PRSM, you can ask the AI Assistant, which will endeavour to help you with instructions and guidance.  The AI Assistant knows everything in this User Guide and also some things about Participatory System Mapping and how to go about mapping.

The Assistant with appear if you click on the button at the lower left corner: <img
src={normalizeImagePath("/images/AIAssistantBtn.png")}
alt="AI HelpAssistant button"
style={{ width: 24, display: "inline" }}
/>.

For other uses of AI within PRSM, see the section on [AI Assistance](/manual/AIAssistance).

## Notes

If you select just one factor or one link, a small panel appears.  This shows the factor or link label (if it has one), the time and date  when the factor or link was created, when it was last modified if it has been changed since it was created, some network statistics about the factor, and an area where you can type a note to show further information about the factor or link.  
...
A basic editor is provided for writing the Notes.  You can format the text with **bold**, *italic* or <u>underline</u>, create bulleted or numbered lists, and add web links (URLs) to the text.
...
Clicking the 'Sparkle' button  <img
src={normalizeImagePath("/images/SparkleIcon.png")}
alt="Sparkle Button"
style={{ width: 24, display: "inline" }}
/> will get AI to generate a draft descriptive note.  For further details see the [AI Assistance](/manual/AIAssistance) section.

For factors, there is also a pushpin symbol <img
src={normalizeImagePath("/images/PinIcon.png")}
alt="Pushpin"
style={{ width: 24, display: "inline" }}
/>. If this is shown solid white, the factor will be locked into place on the network pane and cannot be dragged elsewhere.  The factor will have a small pushpin symbol at the top left.  
...
On the left edge of the map, half way down, is a small tab with three black dots.  Clicking on this opens an **overview panel** where you can describe the map as a whole and enter notes about it. Click on the tab again to close the panel.

<img
src={normalizeImagePath("/images/OverviewPanel.png")}
alt="Overview Panel"
style={{ width: 400}}
/>
...
Click the 'Copy to clipboard' button to copy the web link to the clipboard, and then paste the link into an email or a text message, send them a copy of the QR code or take a photo of it, or just tell other participants the name of the room (which is a randomly generated set of 4 groups of three letters separated by hyphens).  When they access the link, they will see whatever is in your browser window.

There are also three buttons on the right.  If you click  the **Clone map** button, a browser tab will open with the same map but in a new room - any changes made to this new map will *not* appear in the original room.  You can then send the web address to others, confident that they will not disturb the original map.  If you click the **Read only** button, a copy of the map will be made in a new browser tab, but without any of the buttons that would allow one to modify the map. You can use the web link of this map to distribute the map to people whom you want to look at the map, but not change it. If you check the box, 'Allow users to make an editable copy', the map will include a button that will allow them to make a an additional copy which *is* editable.  For more detail about these and about linking and merging maps, see the section on [Cloning, Linking and Merging Maps](/manual/Cloning).

If you click the **Data view** button, a new browser tab will open with a spreadsheet-like view of the factors and links in the map (see the section on the [Data view](/manual/DataView#data-view)).
...
If you didn't provide your real name when you first started using PRSM, your nickname is randomly generated.  If you want to change your name, for instance to use your real name, click on your avatar (the left most one), type in your replacement name and press OK.  
...
Copy and paste is not available on touchscreens.
...
If the dialog appears in an inconvenient place, you can move it by dragging the top header element.  The same applies to all the dialogs in PRSM: they can be moved by dragging their headers.  If you want to cancel all your changes, click on the small '**x**' in the header, or press <kbd>ESC</kbd>.  This also works in other PRSM dialogs.
