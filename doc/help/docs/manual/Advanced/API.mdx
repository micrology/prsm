...
If you wnat to create a new factor or link with a specific design programmatically, it is helpful first to create a factor or link with that design by hand on a map and then use the `GET factor` endpoint to retrieve its details.  You can then use this as a template to create new similar factors or links programmatically.
