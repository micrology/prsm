...
app.post('/api/helpAssistant', chatLimiter, async (req, res) => {
	...
	const region = process.env.AWS_REGION || 'eu-west-2'
	const bedrockApiKey = process.env.BEDROCK_API_KEY
	const kbId = process.env.KNOWLEDGE_BASE_ID || 'PXDZJOQZPQ'
...
function parseHelpAssistantResponse(fullAiResponse) {
	...
	if (structured) {
		...
		return {
			...
			usedIndices: uniqueNonNegativeInts([...structured.usedIndices, ...cleaned.usedIndices]),
...
function normaliseSourceIndexes(value) {
	...
	return uniqueNonNegativeInts(value.map((index) => (typeof index === 'string' ? parseInt(index, 10) : index)))
}
