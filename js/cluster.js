...
function clusterByLouvain() {
	...
	let comms = new Set()
	Object.values(result).forEach((c) => comms.add(c))
	let commColors = {}
	let i = 0
	comms.forEach((c) => {
		commColors[c] = makeColor(i, comms.size)
		i++
	})
	// now cluster the nodes
	unSelect()
	let nodesToUpdate = []
	for (let [nodeId, comm] of Object.entries(result)) {
		let node = data.nodes.get(nodeId)
		if (node.clusteredIn || node.isCluster) continue // already clustered or is a cluster node
		let color = commColors[comm]
		let clusterNode = makeClusterNode(
			`cluster-louvain-${comm}`,
			`Cluster ${comm}`,
			color,
		)
		let sumx = 0
...
function jLouvain(nodes, edges, initialPartition = null) {
	 ...
	 * @param {Object} graph - Graph object with _assoc_mat adjacency matrix
	...
	const getEdgeWeight = (graph, node1, node2) => {
		return graph._assoc_mat[node1]?.[node2]
	}
	 ...
	 * @param {Object} graph - Graph object with _assoc_mat property
	 * @param {Object} edge - Edge object with source, target, and weight properties
	 */
	const updateAssocMat = (graph, { source, target, weight }) => {
		graph._assoc_mat[source] ??= {}
		graph._assoc_mat[source][target] = weight
		graph._assoc_mat[target] ??= {}
		graph._assoc_mat[target][source] = weight
	}
	...
	const computeOneLevel = (graph, status) => {
		...
		while (modif && nbPassDone !== PASS_MAX) {
			curMod = newMod
			modif = false
			nbPassDone += 1
