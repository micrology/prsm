				...
				case "attributeTitles": {
					if (!initialising) {
						...
						for (const attributeFieldName in obj) {
							const colComp = colComps.find(
								(c) => c.getField() === attributeFieldName
							)
							if (colComp) {
...
function setUpShareDialog() {
	...
	function setLink(type) {
		...
		switch (type) {
			case "share":
				path =
					window.location.pathname.replace("table.html", "prsm.html") +
					"?room=" +
					room
				break
			default:
	...
	table.columnManager.columnsByIndex.forEach((col) => {
		if (col.parent.field === field) {
			if (first) {
				first = false
				if (
					document.getElementById(`hide${field}`).dataset.collapsed === "true"
				) {
...
function collapseNotes(table, field = "Notes") {
	const col = table.columnManager.columnsByIndex.filter(
		(c) => c.field === "note"
	)[0]
...
function quillFormatter(cell) {
	...
	if (note) {
		...
		if (
			elem(`hide${table === "factors-table" ? "Notes" : "LinkNotes"}`).dataset
				.collapsed === "false"
		) {
...
function quillAccessor(note) {
	if (note) {
		qed.setContents(note)
		return new QuillDeltaToHtmlConverter(qed.getContents().ops, {
			inlineStyles: true,
		}).convert()
	}
	return ""
...
function convertNode(node) {
	...
	n.leverage =
		n.indegree === 0 ? "--" : (n.outdegree / n.indegree).toPrecision(3)
...
function updateNodeCellData(cell) {
	...
	rows.forEach((row) => {
		...
		const update = {
			id: node.id,
			modifiedTime: timeAndDate(node.modified.time),
		}
		update[field] = value
...
function updateFromAndToLabels(nodes) {
	...
	nodes.forEach((node) => {
		linksToUpdate = linksToUpdate.concat(
			Array.from(yEdgesMap.values()).filter(
...
function updateEdgeCellData(cell) {
	...
	rows.forEach((row) => {
		...
		const update = {
			id: edge.id,
			modifiedTime: timeAndDate(edge.modified.time),
		}
		update[field] = value
		cell.getTable().updateData([update])
		// sync it
...
function setUpFilter() {
	...
	openTable.getColumns().forEach((colComp) => {
		...
		) // cannot sort by color and avoid SVG titles
		{
			select[i++] = new Option(
				def.titleClipboard ||
					(typeof def.title === "string" && def.title[0] !== "<"
						? def.title
						: false) ||
