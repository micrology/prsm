...
function loadExcelfile(contents) {
  ...
  factors.forEach((f) => {
    ...
    if (!f.label) {
      throw new Error(`Factors - Line ${f.__rowNum__}: Factor does not have a Label`)
    }
  ...
  links.forEach((l) => {
    ...
    if (fromFactor) l.from = fromFactor.id
      ...
      throw new Error(`Links - Line ${l.__rowNum__}: From factor (${l.from}) not found for link`)
    }
    const toFactor = factors.find((factor) => factor.label === l.to)
    if (toFactor) l.to = toFactor.id
    else {
      throw new Error(`Links - Line ${l.__rowNum__}: To factor (${l.to}) not found for link`)
    }
