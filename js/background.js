...
export function resizeCanvas() {
	...
	canvas.calcOffset()
	panCanvas(
		(canvas.getWidth() - oldWidth) / 2,
		(canvas.getHeight() - oldHeight) / 2,
		1.0
	)
...
export async function refreshFromMap(keys) {
	if (/back/.test(debug)) {
		keys.forEach((key) =>
			console.log("Key:", key, "value:", yDrawingMap.get(key))
		)
	}
			...
			default: {
				...
				if (localObj) {
					...
					switch (type) {
						...
						default:
							throw new Error(
								`bad fabric object type in yDrawingMap.observe: ${type}`
							)
					}
					localObj.set(otherParams)
					localObj.id = key
...
class CircleHandler extends Circle {
	...
	pointermove(e) {
		...
		const radius =
			Math.sqrt(
				(this.start.x - pointer.x) ** 2 + (this.start.y - pointer.y) ** 2
			) / 2
		this.set({
...
class TextHandler extends IText {
	...
	update() {
		...
		saveChange(
			this,
			{ fontSize: this.fontSize, fill: this.fill, text: this.text },
			"update"
		)
	}
...
function unGroup() {
	...
	saveChange(
		activeObj,
		{ type: "ungroup", members: members.map((ob) => ob.id) },
		"delete"
	)
	canvas.requestRenderAll()
	elem("group").classList.add("disabled")
	...
	undo() {
		...
		if (undo.id === "selection") {
			...
			switch (undo.op) {
				...
				case "update":
					{
						const prevSelParams = undos.findLast(
							(d) => d.id === "selection"
						).params
		...
		if (undo.params.type === "group" || undo.params.type === "ungroup") {
			...
			switch (undo.op) {
				...
				case "update": {
					...
					const newParams = clean(prevDelta.params, {
						type: true,
						layoutManager: true,
					})
					obj.set(newParams)
		...
		if (obj) {
			...
			switch (undo.op) {
				...
				case "update": {
					...
					saveChange(
						obj,
						Object.assign(prevDelta.params, { visible: true }),
						null
					)
					break
				}
			...
			if (obj) {
				switch (redo.op) {
					case "delete":
						...
						saveChange(
							obj,
							{ members: redo.params.members, type: "group" },
							null
						)
						canvas.discardActiveObject()
						updateActiveButtons()
		...
		if (obj) {
			...
			switch (redo.op) {
				...
				case "delete":
					...
					break
				case "update": {
					const newParams = clean(redo.params, {
						type: true,
						layoutManager: true,
					})
					obj.set(newParams)
...
function getObjectData(obj, params) {
	...
	if (obj.type === "path" || obj.type === "Path")
		params.pathOffset = obj.pathOffset
	if (obj.type === "image" || obj.type === "Image")
		params.imageObj = obj.toObject()
...
function intersect(circle, shape) {
	...
	switch (shape[0]) {
		...
		case "text": {
			...
			return (
				(circObj.x - shape[1][1]) ** 2 + (circObj.y - shape[1][2]) ** 2 <
				circObj.r ** 2
			)
		}
		default:
