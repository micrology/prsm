...
const ROLLBACKS = 15 // max. number of versions stored for rollback
...
function draw() {
  ...
  network.on('dragEnd', function (params) {
    ...
    if (params.nodes.length > 0) {
      logHistory(
        `moved factor${params.nodes.length > 1 ? 's' : ''}: '${params.nodes
          .map((id) => shorten(data.nodes.get(id).label))
          .join("', '")}'`
      )
    }
    if (params.edges.length > 0) {
      logHistory(`curved an edge`)
    }
    changeCursor('default')
  })
 ...
 */
function showHistory(smoothScroll = true) {
  ...
  log.innerHTML = yHistory
    ...
    .join(' ')
  Promise.all(
    Array.from(document.querySelectorAll('div.history-rollback')).map((e) => addRollbackIcon(e))
  ).then(() => scrollHistoryToBottom(smoothScroll))
}
/**
 * Scroll the history log wrapper so the most recent entry is visible at the bottom
 */
function scrollHistoryToBottom(smoothScroll) {
  const wrapper = elem('history-log-wrapper')
  const log = elem('history-log')
  requestAnimationFrame(() => {
    wrapper.scrollTo({
      top: log.offsetHeight - wrapper.clientHeight,
...
function showHistorySwitch() {
  if (elem('showHistorySwitch').checked) showHistory(false)
