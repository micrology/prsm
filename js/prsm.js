...
export let debug = ""
console.log(
	`${appName} version ${version} [features: ${features.ai ? "AI" : "no AI"}]`
)
...
window.onbeforeunload = function (event) {
	...
	if (checkMapSaved && dirty) {
		...
		event.returnValue =
			"You have unsaved unchanges.  Are you sure you want to leave?"
	}
}
/**
...
function addEventListeners() {
	...
	listen("nodeStyleEditFactorSize", "input", (event) =>
...
const sliderColor = getComputedStyle(document.documentElement).getPropertyValue(
	"--slider"
)
/**
...
function displayWhatsNew() {
	...
	const versionDecoded = version.match(
...
function startY() {
	...
	if (room == null || room === "") {
		...
		console.log(
			`Generated new room: ${room} and storage is ${sessionStorage.getItem("newRoom")}`
		)
   ...
   */
	yNodesMap.observe((evt) => {
			...
			if (yNodesMap.has(key)) {
				...
				if (data.nodes.get(key)) {
					network
						...
						.forEach((edge) => nodesToRemove.push(edge))
				}
		...
		if (
			/changes/.test(debug) &&
			(nodesToUpdate.length > 0 || nodesToRemove.length > 0)
		) {
   ...
   */
	...
	yEdgesMap.observe((evt) => {
		...
		if (
			/changes/.test(debug) &&
			(edgesToUpdate.length > 0 || edgesToRemove.length > 0)
		) {
...
function draw() {
	...
	const options = {
		...
		manipulation: {
			...
			addEdge: function (item, callback) {
				...
				if (
					data.nodes.get(item.from).isCluster ||
					data.nodes.get(item.to).isCluster
				) {
			...
			editEdge: {
				editWithoutDrag: function (item, callback) {
					...
					const point = network.canvasToDOM({
						x:
							(network.getPosition(item.from).x +
								network.getPosition(item.to).x) /
							2,
						y:
							(network.getPosition(item.from).y +
								network.getPosition(item.to).y) /
							2,
					})
		...
		if (params.nodes.length === 1) {
			...
			if (node.portal && doubleClickTimer === null) {
				doubleClickTimer = setTimeout(() => {
					window.open(
						`${window.location.pathname}?room=${node.portal}`,
						node.portal
					)
	...
	network.on("deselectEdge", function (params) {
		...
		if (params.edges) {
			...
			if (
				prevSelIds.includes(params.edges[0]) ||
				data.edges.get(params.edges[0]).edgeHidden
			) {
	...
	network.on("dragging", function (params) {
		...
		panCanvas(
			viewPosition.x - endViewPosition.x,
			viewPosition.y - endViewPosition.y
		)
...
export function setCanvasBackground(canvas) {
	...
	const backgroundCanvas =
		document.getElementById("underlay").firstElementChild.firstElementChild
...
export function drawMinimap(ratio = 5) {
	...
	function drawMinimapImage() {
		...
		fullNetwork.setOptions({
			edges: { smooth: elem("curveSelect").value === "cubicBezier" },
		})
		fullNetwork.fit()
...
export function toggleDeleteButton() {
	if (
		network.getSelectedNodes().length > 0 ||
		network.getSelectedEdges().length > 0
	) {
	...
	if (showVotingToggle) {
		data.nodes
			...
			.forEach((node) => {
				...
				drawTheBadge(
					node.thumbDown?.includes(clientID)
						? thumbDownFilledImage
						: thumbDownImage,
					ctx,
					box.right - 36,
					box.bottom
...
function copyToClipboard(event) {
	...
	nIds.forEach((nId) => {
		...
		edgesFromNode.forEach((eId) => {
			...
			if (
				nIds.includes(edge.to) &&
				nIds.includes(edge.from) &&
				!edges.find((e) => e.id === eId)
			) {
				edges.push(edge)
			}
		})
...
<!-- Header Labels -->
	...
	elem("node-backgroundColor").style.backgroundColor = standardizeColor(
	...
	elem("node-borderColor").style.backgroundColor = standardizeColor(
		item.color.border
	)
	cp.createColorPicker("node-fontColor")
	elem("node-fontColor").style.backgroundColor = standardizeColor(
		item.font.color
	)
...
function saveNode(item, callback) {
	...
	if (item.shape === "portal") {
		...
		item.shape = "image"
		item.image =
			"data:image/svg+xml;charset=utf-8," + encodeURIComponent(portalSvg)
	}
	if (item.isCluster) {
		item.shape = "image"
	}
...
`
	...
	elem("linkEditLineColor").style.backgroundColor = standardizeColor(
		item.color.color
	)
	elem("linkEditDashes").value = getDashes(item.dashes, null)
	elem("linkEditArrows").value = item.arrows.to.enabled
		? item.arrows.to.type
		: "none"
...
function titleDropDown(title) {
	...
	if (title !== "Untitled map") {
		...
		recentMaps = Object.fromEntries(
			Object.entries(recentMaps).slice(-TITLELISTLEN)
		)
...
// the user is doing
const worker = new Worker(new URL("./betweenness.js", import.meta.url), {
	type: "module",
})
/**
...
function showNodeData(nodeId) {
	...
	if (node.created) {
		elem("nodeCreated").innerHTML =
			`${timeAndDate(node.created.time)} by ${node.created.user}`
		elem("nodeCreation").style.display = "flex"
	} else elem("nodeCreation").style.display = "none"
	if (node.modified) {
		elem("nodeModified").innerHTML =
			`${timeAndDate(node.modified.time)} by ${node.modified.user}`
...
Format everything in Markdown.`
	...
	elem("nodeModified").innerHTML =
		`${timeAndDate(modified.time)} by ${modified.user}`
	positionNotes()
	sparklesElem.classList.remove("rotating")
	if (popupWindow) {
		popupEditor = popupWindow.popupEditor
...
function makeNotesPanelResizeable(notePanel) {
	...
	const notePanelTopLeftHandle = notePanel.querySelector(
...
function showEdgeData(edgeId) {
	...
	if (edge.created) {
		elem("edgeCreated").innerHTML =
			`${timeAndDate(edge.created.time)} by ${edge.created.user}`
		elem("edgeCreation").style.display = "flex"
	} else elem("edgeCreation").style.display = "none"
	if (edge.modified) {
		elem("edgeModified").innerHTML =
			`${timeAndDate(edge.modified.time)} by ${edge.modified.user}`
			...
			case "fan": {
				{
					...
					const selectedNodes = getSelectedAndFixedNodes().map((nId) =>
						data.nodes.get(nId)
					)
...
function selectUsersItems(event) {
	...
	const usersNodes = data.nodes
		.get()
		.filter(
			(n) => n.created?.user === userName || n.modified?.user === userName
		)
		.map((n) => n.id)
	const userEdges = data.edges
		.get()
		.filter(
			(e) => e.created?.user === userName || e.modified?.user === userName
		)
		.map((e) => e.id)
...
function analyse() {
	...
	function hideNodesByRadius(selectedNodes, radius) {
		...
		if (
			getRadioVal("stream") === "upstream" ||
			getRadioVal("stream") === "All"
		) {
			inSet(selectedNodes, radius, "to")
		}
		if (
			getRadioVal("stream") === "downstream" ||
			getRadioVal("stream") === "All"
		) {
		...
		nodeIdsInRadiusSet.forEach((f) => {
			network.getConnectedEdges(f).forEach((e) => {
				...
				if (
					nodeIdsInRadiusSet.has(edge.from) &&
					nodeIdsInRadiusSet.has(edge.to)
				) {
	...
	function hideNodesByStream(selectedNodes, upOrDown) {
		...
		nodeIdsInStreamSet.forEach((f) => {
			network.getConnectedEdges(f).forEach((e) => {
				const edge = data.edges.get(e)
				if (
					nodeIdsInStreamSet.has(edge.from) &&
					nodeIdsInStreamSet.has(edge.to)
				) {
					setEdgeHidden(edge, false)
				}
			})
	...
	function hideNodesByPaths(selectedNodes, pathType) {
		...
		function shortestPaths(selectedNodes, all) {
			...
			const combos = selectedNodes.flatMap((v, i) =>
				selectedNodes.slice(i + 1).map((w) => [v, w])
			)
...
function showAvatars() {
	...
	const nameRecs = recs
		...
		.sort((a, b) =>
			a.name.charAt(0).toUpperCase() > b.name.charAt(0).toUpperCase() ? 1 : -1
		) // sort names
	...
	const cursorsToDelete = Array.from(
	...
	function makeAvatar(nameRec) {
		...
		circle.addEventListener(
			"click",
			nameRec.id === clientID ? renameUser : follow
		)
