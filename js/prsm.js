  ...
  nodes.on("*", (event, properties, origin) => {
    yjsTrace(
      ...
      `${event}  ${JSON.stringify(
        properties.items
      )} origin: ${origin} dontUndo: ${dontUndo}`
    );
    doc.transact(() => {
...
function draw() {
  ...
  function showAreaSelection(event) {
    ...
    selectionArea.style.width = `${
      Math.max(selectionStart.x, event.offsetX) -
      Math.min(selectionStart.x, event.offsetX)
    }px`;
    selectionArea.style.height = `${
      Math.max(selectionStart.y, event.offsetY) -
      Math.min(selectionStart.y, event.offsetY)
    }px`;
  }
  network.on("dragEnd", function (params) {
    ...
    if (e.ctrlKey && params.nodes.length == 0 && params.edges.length == 0) {
      ...
      network.setSelection({
        nodes: selectedNodes
          .map((n) => n.id)
          .concat(network.getSelectedNodes()),
      });
      showSelected();
      return;
    }
  ...
  if (showNotesToggle) {
    ...
    data.nodes
      ...
      .forEach((node) => {
		...
		drawTheBadge(thumbDownImage, ctx, box.right - 20, box.bottom);
     });
...
function editEdge(item, point, cancelAction, callback) {
  ...
  elem("edge-arrow").value = item.arrows.to.enabled
    ? item.arrows.to.type
    : "none";
...
function loadJSONfile(str) {
  ...
  if (json.attributeTitles)
    yNetMap.set("attributeTitles", json.attributeTitles);
...
function setFileName(extn) {
  ...
  lastFileName =
    lastFileName.substring(0, pos < 0 ? lastFileName.length : pos) + "." + extn;
}
/**
 ...
 */
function exportDOT() {
  let str = `/* Creator PRSM ${version} on ${new Date(
    Date.now()
  ).toLocaleString()} */\ndigraph {\n`;
  for (let node of data.nodes.get()) {
    str += `"${node.id}" [label="${node.label}", 
			color="${standardize_color(node.color.border)}", fillcolor="${standardize_color(
      node.color.background
    )}",
			shape="${node.shape == "text" ? "plaintext" : node.shape}",
			${gvNodeStyle(node)},
			fontsize="${node.font.size}", fontcolor="${standardize_color(
      node.font.color
    )}"]\n`;
  }
  for (let edge of data.edges.get()) {
...
function setUpShareDialog() {
  ...
  function openWindow(type) {
    ...
    switch (type) {
      ...
      case "table":
        path =
          window.location.pathname.replace("prsm.html", "table.html") +
          "?room=" +
          room;
        break;
      default:
...
function clone(onlyView) {
  ...
  ws.on("sync", () => {
    ...
    clonedDoc.getArray("history").push([
      {
        action: `cloned this map from room: ${
          room + (onlyView ? " (Read Only)" : "")
        }`,
        time: Date.now(),
        user: myNameRec.name,
      },
...
function keepPaneInWindow(pane) {
  if (
    ...
    pane.style.left =
      container.offsetLeft + container.offsetWidth - pane.offsetWidth + "px";
  }
  if (
...
function showNodeData(nodeId) {
  ...
  if (node.modified) {
    elem("nodeModified").innerHTML = `${timeAndDate(node.modified.time)} by ${
      node.modified.user
    }`;
...
function showEdgeData() {
  ...
  elem("edgeLabel").innerHTML =
    edge.label && edge.label.trim() ? shorten(edge.label) : "Link";
  if (edge.created) {
  ...
  if (edge.modified) {
    elem("edgeModified").innerHTML = `${timeAndDate(edge.modified.time)} by ${
      edge.modified.user
    }`;
...
function displayStatistics(nodeId) {
  ...
  let node = data.nodes.get(nodeId);
  elem("bc").textContent =
    node.bc >= 0 ? parseFloat(node.bc).toPrecision(3) : "--";
}
...
function positionNotes() {
  let notesPanel;
  if (!elem("nodeDataPanel").classList.contains("hide"))
    notesPanel = elem("nodeDataPanel");
  if (!elem("edgeDataPanel").classList.contains("hide"))
    notesPanel = elem("edgeDataPanel");
  ...
  if (
    ...
    notesPanel.style.left =
      settingsRect.left - notesPanelRect.width - 20 + "px";
  }
  if (notesPanelRect.left < netPaneRect.left)
    notesPanel.style.left = netPaneRect.left + 20 + "px";
  if (notesPanelRect.right > netPaneRect.right)
    notesPanel.style.left =
      netPaneRect.right - notesPanelRect.width - 20 + "px";
  if (notesPanelRect.top < netPaneRect.top)
    notesPanel.style.top = netPaneRect.top + 20 + "px";
  if (notesPanelRect.bottom > netPaneRect.bottom)
    notesPanel.style.top =
      Math.max(netPaneRect.bottom - notesPanelRect.height, netPaneRect.top) +
      20 +
      "px";
}
// Network tab
      ...
      case "fan":
        {
          ...
          let selectedNodes = getSelectedAndFixedNodes().map((nId) =>
            data.nodes.get(nId)
          );
...
function analyse() {
  ...
  if (getRadioVal("stream") == "All" && getRadioVal("radius") == "All")
    statusMsg(
      ...
      } between ${listFactors(getSelectedAndFixedNodes(), true)}`
    );
  else
    statusMsg(
      `Factors ${streamMsg} ${
        streamMsg && radiusMsg ? " and " : ""
      } ${radiusMsg} of ${listFactors(
  ...
  function hideNodesByRadius(selectedNodes, radius) {
    ...
    nodeIdsInRadiusSet.forEach((f) => {
      network
        .getConnectedEdges(f)
        .forEach((e) => (data.edges.get(e).hidden = false));
    });
  ...
  function hideNodesByPaths(selectedNodes, pathType) {
    ...
    function shortestPaths(selectedNodes, all) {
      ...
      let combos = selectedNodes.flatMap((v, i) =>
...
async function rollback(event) {
  ...
  if (!rb) return;
  if (
    !confirm(`Roll back the map to what it was before ${timeAndDate(rbTime)}?`)
  )
    return;
    ...
    if (sentpkt) {
      ...
      if (lastPktTime !== sentpkt.time) {
        if (Date.now() - sentpkt.time > SLOWTRIPTIME || /round/.test(debug)) {
          ...
          console.log(
            `${exactTime(sentpkt.time)} Round trip: ${
              Date.now() - sentpkt.time
            } ms`
          );
        }
  ...
  if (elem("showUsersSwitch").checked) {
    ...
    changed.forEach((userId) => {
      ...
      if (
        userId !== clientID &&
        rec.cursor &&
        !object_equals(rec.cursor, lastMicePositions.get(userId))
      ) {
      ...
      if (rec.user) {
        ...
        if (
          refreshAvatars ||
          !object_equals(rec.user, lastAvatarStatus.get(userId))
        )
          showAvatars();
