...
async function genAINode() {
	...
	let systemPrompt = `You are to explain a single factor, A.
...
Format everything in Markdown.`
	let aiResponse = await getAIresponse(`Explain ${node.label}. Use these keywords as context: ${context}`, systemPrompt)
...
async function genAIEdge() {
	...
	let edge = data.edges.get(edgeId)
	let context = data.nodes.get().map(n => n.label.replaceAll('\n', ' ')).join(', ')
	let systemPrompt = `You are to explain a single causal link of the form A causes B.
Follow all the instructions below for your response:
...
Use these keywords as context: ${context}`, systemPrompt)
	editor.setContents(aiResponse)
	let modified = timestamp()
	data.edges.update({
		id: edgeId,
